...
### 2. 한 곳을 고치면 잘 작동하던 다른 곳이 깨졌다

같은 모듈에서 작은 수정 한 번에 무관해 보이던 다른 기능이 깨지는 경우가 있었다. 매번 영향 범위를 머릿속에서만 추측해 작업하다 보니, 반복적으로 확인해야 하는 의존성이나 불변 조건을 빠뜨리기 쉬웠다. **그렇다고 모듈 동작 명세 전체를 매 세션 context에 통째로 넣으면 token budget이 빠르게 소모됐다.**

→ 이런 문제가 잦은 모듈에는 `<module>/SPEC.md`를 둔다. 이 파일에는 동작 명세와 Invariants(의존성 맵 · 불변 조건 · 충돌 감지 규칙)를 기록한다. monorepo에서 작업 대상 모듈이 정해지면 `/dev-docs`가 **그 모듈의 SPEC.md만** 디스패치 `[CONTEXT]`에 자동 포함시키고, Coordinator도 작업 시작 전 체크리스트에서 함께 읽는다. 항상 모든 명세를 들고 있는 대신, 작업 대상 모듈의 명세만 그때그때 주입한다. **1번의 `/clear` + plan/context/tasks 분리와 같은 문제의식이다. 필요한 context만 필요한 순간에 다시 불러온다.**

### 3. 작업 크기에 따라 workflow 무게를 다르게 가져가고 싶었다
...
### 4. Opus 4.7은 판단에, Sonnet/Haiku는 실행에 배치했다
...
### 5. 솔로 작업에서는 commit + history 기록이 더 실용적이었다
...
### 6. 완료 보고 이후 commit / history 누락을 기계적으로 막고 싶었다
